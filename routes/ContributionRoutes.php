...
switch ($method . ' ' . ($pathParts[0] ?? '') . '/' . ($pathParts[1] ?? '')) {
    ...
    case 'GET contribution/all':
        ...
        if (isset($_GET['contribution_type_id']) && is_numeric($_GET['contribution_type_id'])) $filters['contribution_type_id'] = intval($_GET['contribution_type_id']);
    ...
    case 'GET contribution/average':
        ...
        if (!$month) Helpers::sendError('Month required (YYYY-MM)', 400);

        try {
            $result = Contribution::getAverage($month);
            echo json_encode($result);
        } catch (Exception $e) {
            Helpers::sendError($e->getMessage(), 400);
        }
